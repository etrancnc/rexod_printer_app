...
class RealPrinterService extends ChangeNotifier {
  ...
  static const List<int> _CUT_PARTIAL = [0x1D, 0x56, 0x42, 0x03];        // GS V B n (기본: 3줄 급지)
  static const List<int> _CUT_PARTIAL_NO_FEED = [0x1D, 0x56, 0x42, 0x00]; // GS V B 0 (급지 0줄)
  ...
  static const List<int> _ESC_T_KOREAN = [0x1B, 0x74, 30]; // ESC t 30 (KSC-5601/EUC-KR)
  List<int> _escAlign(int n) => [0x1B, 0x61, n];       // 0:left, 1:center, 2:right
  List<int> _ensureKR() => [
    0x1C, 0x26,       // FS &  (KANJI/멀티바이트 모드 진입)
    0x1B, 0x74, 30,   // ESC t 30 (KSC-5601 / EUC-KR)
  ];
  ...
  Future<bool> connect() async {
    ...
    try {
      ...
      if (_config.port == 'USB') {
        ...
        if (!okOpen) throw Exception('스풀러 열기 실패: $_spoolPrinterName');
        ...
        _status = _status.copyWith(mechanicalType: 'RMP8300 (USB)', sdkVersion: 'WinSpool');
        _statusKnown = false;
        _isConnecting = false;
        notifyListeners();
        return true;
        ...
        if (!_port!.isOpen) throw Exception('포트가 열리지 않았습니다.');
        ...
        if (!ok) throw Exception('프린터가 응답하지 않습니다. 전원/케이블/포트를 확인하세요.');
  ...
  Future<bool> printQRCode(String data, {int moduleSize = 6, int ecLevel = 0x31}) async {
    if (!(_useSpooler ? (_spool?.isOpen ?? false) : (_port?.isOpen ?? false))) {
      _showMessage('프린터가 연결되지 않았습니다.', MessageType.error);
      return false;
    }
    ...
    try {
      ...
      final bytes = <int>[
        0x1D, 0x28, 0x6B, 0x04, 0x00, 0x31, 0x41, 0x32, 0x00,         // 모델2
        0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x43, size,                // 모듈사이즈
        0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x45, ecc,                 // 오류정정
        0x1D, 0x28, 0x6B, pL, pH, 0x31, 0x50, 0x30, ...bytesData,      // 데이터
        0x1D, 0x28, 0x6B, 0x03, 0x00, 0x31, 0x51, 0x30,                // 인쇄
        ..._LINE_FEED,
      ];
  ...
  Future<bool> printSampleReceipt() async {
    ...
    try {
      ...
      final qrBlock = await _buildAsciiQrBytesNoMargin(
        asciiArt.toString(),
        columns: 48,
        cp437Index: 0,
      );
      ...
      b.add(_INIT);
      b.add(_escSelectCodePage(30));   // EUC-KR
      b.add(bodyKR);
      b.add(_ensureKR()); // 안전복귀
      // QR (가운데 + 위/아래 여백 없음)
      b.add(qrBlock);

      // 이후 텍스트 필요 시 추가 (예: 한 줄만)
      b.add(_LINE_FEED);

      // 마무리
      b.add(_LINE_FEED);
      b.add(_CUT_PARTIAL_NO_FEED);
  ...
  String _rtrimNewlines(String s) => s.replaceFirst(RegExp(r'[\r\n]+$'), '');
  int _skipLeadingNewlines(String s, int start) {
    var i = start;
    while (i < s.length) {
  ...
  // ===== QR ASCII 전용 빌더: 가운데/무여백/줄간격 타이트 + EUC-KR 복귀 =====
  Future<List<int>> _buildAsciiQrBytesNoMargin(
      String ascii, {
        required int columns,
        int cp437Index = 0,

        // === 미세 조정 파라미터 ===
        int centerBias = 0,   // -2..+2 정도로 좌/우 미세 이동
        int lfBefore = 1,     // QR 위쪽 줄바꿈 개수
        int lfAfter  = 1,     // QR 아래쪽 줄바꿈 개수
        int tight    = 24,    // QR 출력 시 줄간격(기본 22~26 사이 추천)
      }) async {
    var norm = ascii.replaceAll('\r', '');
    norm = norm.replaceFirst(RegExp(r'^\s*\n+'), '');
    norm = norm.replaceFirst(RegExp(r'\n+\s*$'), '');

    final lines = norm.split('\n');
    final maxW  = lines.fold<int>(0, (m, l) => l.length > m ? l.length : m);

    final pad = (((columns - maxW) ~/ 2) + centerBias).clamp(0, 1000);
    final leftPad = ' ' * pad;
    final centered = lines.map((l) => '$leftPad$l').join('\n');

    final bytes437 = await _encodeCP437(centered);

    final b = BytesBuilder()
      ..add(List.filled(lfBefore, 0x0A))   // QR 위 공백 줄
      ..add([0x1C, 0x2E])                  // FS . : KANJI 해제(1바이트 안전)
      ..add([0x1B, 0x74, cp437Index])      // ESC t n : CP437
      ..add([0x1B, 0x61, 0x00])            // 좌측정렬(패딩으로 중앙 맞춤)
      ..add([0x1B, 0x33, tight])          // 줄간격 타이트
      ..add(bytes437)
      ..add(List.filled(lfAfter, 0x0A))    // QR 아래 공백 줄
      ..add([0x1B, 0x32])                  // 줄간격 기본 복귀
      ..add(_ensureKR());                  // 한글(EUC-KR) 복귀
  ...
  Future<bool> printTicketInlineFromUrl({
    required String url,
    int columns = 48,   // 58mm: 32/42, 80mm: 48/64
    int cp437Index = 0, // 모델 따라 0 또는 16
  ...
  /// JSON 기반: [TICKET] 자리에 QR 아스키아트 삽입(중앙/무여백), 이후 한글 정상 출력
  Future<bool> printTicketInlineFromJson(
      Map<String, dynamic> j, {
        int columns = 48,   // 58mm: 32/42, 80mm: 48/64
        int cp437Index = 0, // 일부 모델은 16이 CP437
      }) async {
    if (!isConnected) {
      _showMessage('프린터가 연결되지 않았습니다.', MessageType.error);
      return false;
    ...
    try {
      ...
      String body = receipts.isNotEmpty && receipts.first.trim().isNotEmpty ? receipts.first : '';
      body = _sanitizeBody(body);

      // 2) QR 매핑
      final qrList = (j['qr_codes'] as List?)?.whereType<Map<String, dynamic>>().toList() ?? const [];
      final Map<String, String> qrMap = {
        for (final m in qrList) (m['coupon_type'] ?? '').toString(): (m['qr'] ?? '').toString(),
      };
      ...
      for (final m in matches) {
        // (a) 플레이스홀더 이전의 본문 → 뒤쪽 개행 rstrip해서 붙임 (QR 윗공백 제거)
        var before = body.substring(last, m.start);
        if (before.isNotEmpty) {
          before = _rtrimNewlines(before);
          b.add(await _encodeKR(before));
        }

        // (b) QR 블럭 (무여백)
        final key = m.group(1) ?? '';
        final qrArt = qrMap[key] ?? '';
        if (qrArt.isNotEmpty) {
          final qrBlock = await _buildAsciiQrBytesNoMargin(
            qrArt,
            columns: columns,
        ...
        // (c) 플레이스홀더 직후의 개행은 통째로 스킵 → 아래 공백 제거
        last = _skipLeadingNewlines(body, m.end);
      }
      ...
      _showMessage(ok ? '티켓 인쇄 완료' : '티켓 인쇄 실패', ok ? MessageType.success : MessageType.error);
  ...
  Future<void> _updatePrinterStatus() async {
    if (_useSpooler) return;
    try {
      if (!await _sendCommand(_STATUS_REQUEST)) {
        throw Exception('상태 요청 전송 실패');
      }
      await Future.delayed(const Duration(milliseconds: 200));
      _status = _status.copyWith(mechanicalType: 'RMP8300', sdkVersion: 'v2.1.0');
  ...
  Future<bool> rebootPrinter() async {
    ...
    try {
      final ok = await _sendRaw([0x1B, 0x3F, 0x0A, 0x00], docName: 'REBOOT'); // 데모
